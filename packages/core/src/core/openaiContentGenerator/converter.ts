    ...
    if (choice) {
      ...
      if (choice.finish_reason) {
        ...
        for (const toolCall of completedToolCalls) {
          ...
          // If parsing failed, surface an error response instead of emitting a malformed call.
          if (toolCall.parseError) {
            parts.push({
              functionResponse: {
                ...
                name: toolCall.name,
                response: {
                  error: `Failed to parse arguments for tool "${toolCall.name}". Please resend a valid JSON object with all required parameters. Details: ${toolCall.parseError}`,
                },
              },
            });
            continue;
