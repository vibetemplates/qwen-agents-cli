...
export class StreamingToolCallParser {
  ...
  /** Track parse errors encountered for each tool call index */
  private parseErrors: Map<number, Error> = new Map();
  ...
  addChunk(
    ...
    if (depth === 0 && newBuffer.trim().length > 0) {
      try {
        ...
        this.parseErrors.delete(actualIndex);
        ...
        if (e instanceof Error) {
          this.parseErrors.set(actualIndex, e);
        }
        return {
          complete: false,
        ...
        try {
          ...
          if (inString) {
            try {
              ...
              parseError =
                this.parseErrors.get(index)?.message ||
