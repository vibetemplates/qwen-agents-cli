...
export class GeminiClient {
  ...
  /**
   * Recreate the chat with a new system instruction while preserving history and tools.
   * Used when the system prompt changes mid-session.
   */
  async resetChatWithSystemInstruction(sysInstr: string): Promise<void> {
    const existingHistory = this.chat?.getHistory() ?? [];
